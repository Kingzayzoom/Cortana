...
if (!["--check", "--apply"].includes(mode)) {
  console.error("Usage: node --env-file=.env.local scripts/apply-email-summary-migration.mjs --check|--apply");
  process.exit(2);
}

const { SUPABASE_URL, SUPABASE_PROJECT_REF, SUPABASE_ACCESS_TOKEN } = process.env;
if (!SUPABASE_URL || !SUPABASE_PROJECT_REF || !SUPABASE_ACCESS_TOKEN)
  throw new Error("Supabase URL, project ref, and Management API access token are required.");
const projectHost = new URL(SUPABASE_URL).hostname;
if (!projectHost.endsWith(".supabase.co") || projectHost.split(".")[0] !== SUPABASE_PROJECT_REF)
  throw new Error("Supabase project ref does not match SUPABASE_URL. No SQL was sent.");
...
async function query(sql, readOnly) {
  ...
  if (!response.ok) {
    ...
    console.error(`Management API error: ${String(data?.message ?? data?.error ?? "unknown").slice(0, 500)}`);
    process.exit(1);
  }
  return data;
}
...
if (mode === "--apply") {
  const migration = new URL("../supabase/migrations/20260919000000_email_summary.sql", import.meta.url);

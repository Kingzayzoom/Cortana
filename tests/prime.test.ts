...
describe("Prime persistence", () => {
  it("reinforces a miss with a different question and awards its bonus once", async () => {
    const first = await actPrime("reinforce", { action: "start" }, now),
      q = questions.find((q) => q.id === first.questions[0].id)!;
    await actPrime(
      "reinforce",
      {
        action: "answer",
        ...
        questionId: q.id,
        answer: q.options.find((o) => o.id !== q.correctOptionId)!.id,
      },
      now,
    );
    await finish("reinforce", now);
    const tomorrow = new Date("2026-09-20T16:00:00Z"),
      next = await actPrime("reinforce", { action: "start" }, tomorrow);
    ...
    expect(done.session!.xp).toBe(90);
    expect(done.stats.reinforced).toBe(1);
    await actPrime(
      "reinforce",
      { action: "complete", sessionId: done.session!.id },
      tomorrow,
    );

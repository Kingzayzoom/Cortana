...
describe("mock draft voice prompt", () => {
  it("asks for a message, then transcribes it for the draft", () => {
    ...
    act(() => fireEvent.click(screen.getByRole("button", { name: "Ask Cortana to draft by voice" })));
    ...
    act(() => FakeRecognition.latest?.onresult?.({
      results: [[{ transcript: "Tell the staff I'm running 20 minutes late" }]],
    }));
    expect(onMessage).toHaveBeenCalledWith("Tell the staff I'm running 20 minutes late");
  });
  ...
  it("offers typing when neither speech recognition nor recording is available", () => {
    ...
    expect((screen.getByRole("button", { name: "Ask Cortana to draft by voice" }) as HTMLButtonElement).disabled)
      .toBe(true);
  });
  ...
  it("records a short message for transcription when Firefox has no SpeechRecognition", async () => {
    class FakeRecorder {
      ...
      stop() {
        this.state = "inactive";
        this.ondataavailable?.({ data: new Blob(["x".repeat(600)], { type: "audio/ogg" }) });
        this.onstop?.();
      }
    }
    class FakeUtterance {
    ...
    const fetchMock = vi.fn(async () => Response.json({ text: "Tell the staff I'm running late" }));
    vi.stubGlobal("MediaRecorder", FakeRecorder);
    vi.stubGlobal("SpeechSynthesisUtterance", FakeUtterance);
    vi.stubGlobal("speechSynthesis", { speak: (prompt: FakeUtterance) => prompt.onend?.() });
    ...
    fireEvent.click(screen.getByRole("button", { name: "Ask Cortana to draft by voice" }));
    fireEvent.click(await screen.findByRole("button", { name: "Stop listening" }));
    await waitFor(() => expect(onMessage).toHaveBeenCalledWith("Tell the staff I'm running late"));
    expect(fetchMock).toHaveBeenCalledWith("/api/email-summary/transcribe", expect.objectContaining({
      method: "POST",
    }));
  });
});

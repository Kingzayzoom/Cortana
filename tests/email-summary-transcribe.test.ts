...
describe("Firefox voice transcription route", () => {
  it("sends a short audio clip to Gemini and returns only the transcript", async () => {
    ...
    const gemini = vi.fn(async () => Response.json({ candidates: [{ content: { parts: [
      { text: '{"transcript":"Tell staff I am running late"}' },
    ] } }] }));
    vi.stubGlobal("fetch", gemini);
    const form = new FormData();
    form.append("audio", new Blob(["a".repeat(600)], { type: "audio/ogg" }), "voice.ogg");
    const response = await POST(new Request("http://localhost:3000/api/email-summary/transcribe", {
      method: "POST",
    ...
    expect(await response.json()).toEqual({ text: "Tell staff I am running late" });
    expect(gemini).toHaveBeenCalledOnce();
    const [url, options] = gemini.mock.calls[0] as unknown as [string, RequestInit];
    ...
    expect(Buffer.from(body.contents[0].parts[1].inlineData.data, "base64").length).toBe(600);
  });
});

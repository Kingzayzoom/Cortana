...
function Scene({
  ...
  const perlinNoiseTexture = useTexture("/orb-noise.png");
  ...
  useFrame((_, delta: number) => {
    ...
    if (u.uOpacity.value < 1) {
      u.uOpacity.value = reducedMotion
        ? 1
        : Math.min(1, u.uOpacity.value + dt * 2);
    }
    ...
    u.uColor2.value.lerp(targetColor2Ref.current, 1 - Math.exp(-dt * 5));
  });
  ...
  const uniforms = useMemo(() => {
    ...
    return {
      ...
      uTime: new THREE.Uniform(12),
      uAnimation: new THREE.Uniform(1.2),
...
void main() {
    ...
    // Upstream's flat two-color polar finish cannot express the requested
    // translucent sphere. Preserve its noise/flow and add a spherical surface,
    // refraction-like ribbons, and controlled specular highlights.
    float z = sqrt(max(0.0, 1.0 - radius * radius));
    vec3 normal = normalize(vec3(uv, z));
    float energy = max(uInputVolume, uOutputVolume);
    ...
    vec3 blue = mix(uColor1, vec3(0.18, 0.48, 1.0), 0.48);
    vec3 violet = mix(uColor2, vec3(1.0, 0.078, 0.725), 0.19);
    vec3 cyan = vec3(0.25, 0.89, 1.0);
    vec3 material = mix(blue, violet, smoothstep(-0.65, 0.85, ribbon));
    material = mix(material, cyan, smoothstep(0.2, 0.95, fold) * 0.8);
    float light = 0.66 + 0.34 * max(dot(normal, normalize(vec3(-0.7, 0.9, 1.3))), 0.0);
    material *= light;
    float ribbonEdge = pow(1.0 - abs(ribbon), 16.0) * 0.24;
    material += vec3(0.50, 0.8, 1.0) * ribbonEdge;
    vec2 highlightUv = mat2(0.84, -0.54, 0.54, 0.84) * (uv - vec2(-0.42, 0.49));
    float specular = exp(-pow(highlightUv.x / 0.095, 2.0) - pow(highlightUv.y / 0.34, 2.0));
    float sheen = pow(max(dot(normal, normalize(vec3(-0.42, 0.6, 1.0))), 0.0), 9.0);
    material = mix(material, vec3(0.93, 0.99, 1.0), min(0.98, specular * 0.96 + sheen * 0.34));
    float lowerReflection = exp(-30.0 * length(uv - vec2(0.3, -0.72)));
    material += vec3(0.35, 0.66, 1.0) * lowerReflection;
    float rim = pow(1.0 - z, 3.0);
    material = mix(material, vec3(0.86, 0.97, 1.0), rim * 0.9);
    float edgeLight = smoothstep(0.952, 0.981, radius) * (1.0 - smoothstep(0.987, 1.0, radius));
    material = mix(material, vec3(0.93, 0.99, 1.0), edgeLight * 0.65);
    float grain = (hash2(vUv * 900.0).x - 0.5) * 0.013;
    color.rgb = pow(max(material + grain, vec3(0.0)), vec3(0.79));
    color.a = 1.0 - smoothstep(0.985, 1.0, radius);

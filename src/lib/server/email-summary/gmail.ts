...
export async function gmailGet(
  ...
  if (!response.ok)
    throw new RequestError("Gmail could not load recent inbox messages.", 502);
...
async function accessTokenFor(connection: Connection) {
  ...
  const granted = new Set(
    token.success && token.data.scope
  ...
  if (
    !token.success ||
    (token.data.scope &&
      (!granted.has(GMAIL_SCOPE) ||
        [...granted].some((scope) => !knownScopes.has(scope))))
  )
    throw new RequestError(
      "Reconnect Gmail to renew the authorized access.",
      409,
    );
...
export function sourceFromGmailMessage(
  value: unknown,
  now = Date.now(),
): SourceMessage | null {
  ...
  if (
    ...
    message.labelIds.some((label) => excludedLabels.has(label))
  )
    return null;
  ...
  if (
    ...
    headers.has("list-id") ||
    /^(bulk|list)$/i.test(headers.get("precedence")?.trim() ?? "") ||
    /\bnewsletter\b/i.test(
  ...
  if (
    ...
    now - date.getTime() > 30 * 60 * 60 * 1000 ||
...
export async function fetchRecentInbox(
  ...
  const listing = z
    ...
    .parse(
      await gmailGet("messages", accessToken, {
  ...
  for (
    let index = 0;
    index < Math.min(listing.messages.length, MAX_MESSAGES);
    index += 5
  ) {
  ...
  return details
    .map((value) => sourceFromGmailMessage(value))

...
function dbUrl(table: "email_connections" | "email_summaries", query?: Record<string, string>) {
...
const connectionSelect = "id,email_address,access_token_ciphertext,refresh_token_ciphertext,access_expires_at";

export async function connectionById(id: string) {
  return firstConnection(await dbRequest("email_connections", "GET", {
    id: `eq.${id}`, select: connectionSelect, limit: "1",
  }));
}
...
export async function saveConnection(args: {
  ...
  return firstConnection(await dbRequest("email_connections", "POST", {
    ...
    access_expires_at: new Date(Date.now() + args.expiresIn * 1000).toISOString(),
    updated_at: now,
  }));
}

export async function updateTokens(connection: Connection, args: {
  accessToken: string;
  refreshToken?: string;
  expiresIn: number;
}) {
  await dbRequest("email_connections", "PATCH", { id: `eq.${connection.id}` }, {
    access_token_ciphertext: encryptToken(args.accessToken),
    refresh_token_ciphertext: args.refreshToken
      ? encryptToken(args.refreshToken) : connection.refresh_token_ciphertext,
    access_expires_at: new Date(Date.now() + args.expiresIn * 1000).toISOString(),
...
export async function latestSummary(connectionId: string, date?: string): Promise<EmailSummary | null> {
  const rows = z.array(z.object({ summary_json: z.unknown() })).parse(
    await dbRequest("email_summaries", "GET", {
      ...
      select: "summary_json", order: "generated_at.desc", limit: "1",
    }),
  );
...
export async function saveSummary(
  ...
  await dbRequest("email_summaries", "POST", {
...
export async function listConnections() {
  ...
  for (let offset = 0; ; offset += 500) {
    const page = z.array(connectionSchema).parse(await dbRequest("email_connections", "GET", {
      select: connectionSelect, order: "id.asc", limit: "500", offset: String(offset),
    }));
...
export function decryptToken(value: string) {
  ...
  if (version !== "v1" || !iv || !tag || !encrypted)
    throw new RequestError("Reconnect Gmail to renew the saved authorization.", 409);
  try {

...
export async function emailSummaryStatus(): Promise<EmailSummaryStatus> {
  ...
  if (!connection) return {
    connected: false, emailAddress: null, summary: null, schedule,
  };
  return {
    connected: true,
...
export async function generateOwnSummary() {
  const connection = await connectedAccount();
  if (!connection) throw new RequestError("Connect Gmail before refreshing the briefing.", 401);
...
export async function generateDailySummaries() {
  ...
  for (let index = 0; index < connections.length; index += 3) {
    ...
    const results = await Promise.allSettled(batch.map((connection) => generateSummary(connection, true)));
